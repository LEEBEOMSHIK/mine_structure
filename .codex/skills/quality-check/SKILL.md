...
> **제작 직후 필수 단계.** 가구/스킨/의상을 새로 만들거나 수정한 뒤에는 커밋 전에 이 스킬을 반드시 거친다(제작 파이프라인의 마지막 검수 단계). 별도 요청이 없어도 자동으로 적용한다.
...
4. **수정 루프 (최대 2회 redo)**
   ...
   - 수정 후 **1~3을 다시** 돌린다. **재수정(redo)은 한 항목당 최대 2회까지만** 한다.
   - 2회 redo 후에도 남는 **경미한** 문제는 더 고치지 말고 "알려진 한계"로 **보고만 하고 통과(OK)** 처리한다(무한 루프 방지). **치명적** 문제(렌더 깨짐·완전히 다른 형태 등)만 사용자에게 에스컬레이션한다.
