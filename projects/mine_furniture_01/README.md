...
Minecraft Bedrock용 **여러 가구와 무기**를 묶는 add-on 프로젝트다. 현재 유니콘 테마 가구 라인으로 **무지개 유니콘 변기(`unicorn_toilet`)**, **유니콘 식탁(`unicorn_dining_table`)**, **유니콘 의자(`unicorn_chair`)**를 관리한다.
...
1. Minecraft에서 유니콘 가구 3종 엔티티 소환을 테스트한다.
   - `dist/mine_furniture_01-20260601-015204/mine_furniture_01.mcaddon`을 Minecraft Bedrock으로 가져온다.
   - behavior pack/resource pack을 같은 테스트 월드에 활성화한다.
   - 치트가 켜진 월드에서 `/summon mine_structure:unicorn_toilet`, `/summon mine_structure:unicorn_dining_table`, `/summon mine_structure:unicorn_chair`를 실행한다.
   - 기대 결과: 세 엔티티가 깨진 텍스처 없이 보이고, 충돌 박스가 각 가구 크기에 맞게 동작한다.
...
5. 추후 최종 제작/제공 음원이 생기면 `addon/resource_pack/sounds/flush.ogg`를 교체하고 사운드 검증을 다시 실행한다.
6. 필요하면 lid_open/lid_close/flush 애니메이션을 Blockbench 모델 안에도 빌드해 scaffold와 일원화할지 결정한다.
