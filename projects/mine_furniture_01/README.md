...
| `mine_structure:unicorn_wand` | tool item | 마법 지팡이, 우클릭 파티클+점프/스피드 부스트, in-game test 대기 | `content/items/unicorn_wand.md` |
...
| `mine_structure:unicorn_wings` | wearable | 유니콘 날개(가슴 착용 코스튬), in-game test 대기(실험) | `content/wearables/unicorn_costume.md` |
...
- 👑 **입는 코스튬 2종**(`unicorn_horn_headband` 머리 슬롯 / `unicorn_wings` 가슴 슬롯) — `minecraft:wearable` + attachable. attachable geometry의 본 이름을 플레이어 본(`head`/`body`)에 맞춰 착용 시 캐릭터에 3D로 보이게 했다. **⚠️ 플레이어 부착 커스텀은 버전 민감 → 인게임 검증 필수(가장 불확실).**
...
16. 인게임에서 음식 세트/지팡이/블록/코스튬을 검증한다.
    ...
    - **코스튬(가장 불확실)**: `/give` 뿔 머리띠·날개를 받아 머리/가슴 슬롯에 착용 시 캐릭터에 3D로 보이는지. 안 보이거나 위치가 어긋나면 attachable/지오메트리(본 이름·피벗·render controller) 보정 필요.
