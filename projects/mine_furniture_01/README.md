...
- 외형: **2D 인벤토리 아이콘 + 3D 손모델(attachable) 둘 다**.
  ...
  - 3D 모델: Blockbench `furniture` 프로젝트에 `unicorn_horn_blade` 루트 그룹 추가. 폼멜+라벤더 그립+무지개 가드+베이스 글로우+6단 테이퍼 뿔 날(Y축 12°→112° 비틀어 나선). 1 bone / 10 cubes. 월드 오프셋 `[120, 0, 0]`, `geometry.unicorn_horn_blade`는 중심좌표로 직접 작성.
...
- `validate_unicorn_toilet_resources.py`에 무기 검증(`WEAPONS`, `validate_weapon`)과 Blockbench 루트 그룹 검증을 추가했다. 현재 결과 PASS.
- 칼날 큐브에 전용 아틀라스를 적용하고 box UV를 geo.json과 동일하게 맞춰 Blockbench 미리보기에서도 색이 보이게 했다. 아틀라스는 전체 불투명 + 큰 색 블록이라 흰색 샘플이 없다.
- **Blockbench 저장 주의**: 이 환경의 project 저장 코덱이 모든 면 텍스처를 "선택된 텍스처" 하나로 평탄화하는 버그가 있다. `furniture.bbmodel` 저장 직후 `blockbench/fix_bbmodel_face_textures.py`를 실행해 면 텍스처 바인딩을 모델별로 복구한다(각 가구→자기 아틀라스, 칼날→horn_blade). 인게임 리소스는 영향 없음.
