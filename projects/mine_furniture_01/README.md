...
| `mine_structure:unicorn_sink_l` | furniture entity | model/texture/add-on ready, water on/off toggle, in-game test 대기 | `content/furniture/unicorn_sinks.md` |
| `mine_structure:unicorn_sink_island` | furniture entity | model/texture/add-on ready, water on/off toggle, in-game test 대기 | `content/furniture/unicorn_sinks.md` |
| `mine_structure:unicorn_sink_u` | furniture entity | model/texture/add-on ready, water on/off toggle, in-game test 대기 | `content/furniture/unicorn_sinks.md` |
...
10. 인게임에서 싱크대 3종(`/summon mine_structure:unicorn_sink_l`, `unicorn_sink_island`, `unicorn_sink_u`)을 소환해 형태/텍스처를 확인하고, 우클릭으로 물 켜기/끄기 토글(낙수 + 고인물 표시/숨김, 상태 지속)이 동작하는지 검증한다. 필요하면 물 위치/수도꼭지 포즈, collision_box 크기를 미세조정한다.
