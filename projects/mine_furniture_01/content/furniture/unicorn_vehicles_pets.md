...
| `mine_structure:unicorn_pegasus` | mount | 탑승 비행(수평 조종 + 점프/웅크림 상하) | 날개 펄럭임, 공중 호버 |
| `mine_structure:unicorn_fridge` | furniture | 문 열기 애니 + 저장/꺼내기 | 우클릭 문 열림, barrel식 수납(18칸) |
...
- **하늘 나는 유니콘**: `minecraft:rideable`(`controlling_seat 0`, 좌석 `[0,1.0,0]`) + `behavior.controlled_by_player` + `movement.fly`/`navigation.fly`/`flying_speed`(0.6) + `has_gravity:false`(공중 호버). 수평은 탑승자 시선 방향으로 이동, 상하는 `scripts/main.js` runInterval이 탑승 플레이어의 `isJumping`(상승)/`isSneaking`(하강)을 읽어 `applyImpulse`로 처리한다(속도 클램프 ±0.55). 날개 `wing_l`/`wing_r`는 `scripts.animate: ["flap"]`로 항상 펄럭인다. (※ 상하 비행은 Bedrock 네이티브 한계라 Script 보조이며 인게임 튜닝 필요.)
- **냉장고**: `minecraft:interact` → `mine_structure:open_fridge` → `queue_command`로 `playanimation`(경첩 `door` 본 문 열림). 저장은 barrel 수납장 로직 재사용 — `storeOrRetrieveItem(player, entity, "fridge_items", 18)`로 아이템 들고=저장 / 빈손=꺼내기. 문 애니(behavior)와 저장(Script)은 한 우클릭에서 각자 동작.
