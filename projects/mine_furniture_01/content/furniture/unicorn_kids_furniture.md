...
| `mine_structure:unicorn_cloud_bunk_bed` | 구름 2층침대 | `minecraft:rideable`(2인) | 그냥 우클릭=아래칸 / 웅크리고 우클릭=위칸. 좌석 `[0,0.5,0]`/`[0,1.3,0]` |
...
- **구름 2층침대**: rideable 좌석 2개(아래/위 칸). 두 플레이어가 각각 앉을 수 있고, **혼자서도 칸을 고를 수 있다** — 그냥 우클릭=아래칸, 웅크리고(Shift) 우클릭=위칸. Bedrock은 좌석을 낮은 번호부터 채우므로, 같은 두 좌석을 순서만 바꾼 component group(`order_bottom`=[아래,위] / `order_top`=[위,아래])을 두고, 웅크림 우클릭 시 `scripts/main.js`(`rideBunkTop`)가 `order_top`으로 바꿔 위칸에 다시 앉힌다. 빈 침대는 runInterval이 `order_bottom`으로 되돌려 다음 솔로가 아래칸부터 쓰게 한다(`bunk_seatlock` dynamic property로 전환 중 되돌림 방지). (riding은 앉는 자세이며 바닐라엔 눕는 포즈가 없다.)
