...
현재 등록 콘텐츠는 가구/펫 16종 — **무지개 유니콘 변기(`mine_structure:unicorn_toilet`)**, **유니콘 식탁(`mine_structure:unicorn_dining_table`)**, **유니콘 의자(`mine_structure:unicorn_chair`)**, **유니콘 barrel 수납장(`mine_structure:unicorn_barrel_cabinet`)**, **장식용 유니콘 인형(`mine_structure:decorative_unicorn_doll`)**, **유니콘 싱크대 3종(`mine_structure:unicorn_sink_l` / `unicorn_sink_island` / `unicorn_sink_u`)**, **아이 친화적 가구 4종(`mine_structure:unicorn_rocking_horse` 흔들목마 / `unicorn_night_lamp` 무드등 / `unicorn_ice_cream_machine` 아이스크림 기계 / `unicorn_cloud_bunk_bed` 구름 2층침대)**, **펫/놀이 4종(`unicorn_baby_pet` 아기 유니콘 펫 / `unicorn_gacha_machine` 가챠 / `unicorn_trampoline` 트램폴린 / `unicorn_gift_box` 선물상자)** — 과 무기 1종 **유니콘 뿔 검(`mine_structure:unicorn_horn_blade`)**이다.
