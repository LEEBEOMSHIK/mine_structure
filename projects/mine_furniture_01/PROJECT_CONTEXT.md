...
- (2026-06-08) 커스텀 날개 `unicorn_elytra`를 **나비 날개**로 재디자인(천사 깃털에서 변경). `butterfly_edge`(둥근 forewing+hindwing) + `draw_wing`(forewing 분홍→라벤더/hindwing 하늘→민트 그라데이션, 날개맥, 점박이) + `spot`(eyespot 2개). 해상도 32×64(atlas 128), 두께 0.05, 이름 "나비 날개". Blockbench 확인, 검증 PASS. 최신 패키지: `../../dist/mine_furniture_01-20260608-132640/mine_furniture_01.mcaddon`
